...
st.set_page_config(page_title="自定义对话", layout="wide")  # 配置页面的默认设置

st.title("基于自定义混合对话")   # 在应用程序中显示一个标题，设置标题的内容
...
with st.container():                    # 创建一个容器，用于组织下面的内容
    ...
    for message in st.session_state["messages3"]:            # 遍历st.session_state["messages"]中的消息
        i = i+1
        if i % 2 == 1:
    ...
    if prompt:                                              # prompt不为空None时
        st.session_state["messages3"].append(prompt)         # 将用户输入的消息添加到st.session_state["messages"]中
        start_time = time.time()                            # 开始计时
        ai_message = query_KG_and_Vector(prompt)                   # 使用chat对象对用户输入的消息进行处理，并将返回的助手消息赋值给ai_message变量
        end_time = time.time()
        load_time = end_time - start_time
        st.session_state["messages3"].append(ai_message)     # 将助手消息添加到st.session_state["messages"]列表中。
